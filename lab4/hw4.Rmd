...
ggplot(data=msleep,
  mapping=aes(x=vore))+
  geom_bar(mapping=aes(fill=vore)) #herbivore most represented, vore is categorical so it is x
```
...
```{r}
ggplot(data=msleep, #specifies the data
       mapping = aes(x=vore,y=bodywt))+#map the aesthetics
  scale_y_log10()+
  geom_boxplot()
```
...
ggplot(data=msleep,
  ...
  labs(title="Body Weight (kg) vs Sleep Total (hrs)",
       x="Body Weight (kg)", y= "sleep total (hrs)")
#as weight increases, sleep decreases
```
...
```{r}
ggplot(data=msleep, #specifies the data
       mapping = aes(x=vore,y=sleep_total))+#map the aesthetics
  scale_y_log10()+
  geom_boxplot()
#insectivore sleeps the most
```
...
#as body weight increases, brain weight does too
```
...
ggplot(data=msleep,
  ...
  geom_point(mapping=aes(color=sleep_rem))+
  geom_smooth(method=lm, se=T)+
  labs(title="Sleep rem vs sleep total in hours",
       x="sleep total (hrs)", y= "sleep rem (hrs)")
#as sleep rem increases, sleep total does too
```
