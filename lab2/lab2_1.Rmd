...
Code chunk: press on C+ the green button, hold opt and command and push "I", or copy paste previous code chunk

getwd() = when run, where u r on the computer, the link it gives u is the working directory
run = cmd + return
'#' in codes are comments, doesnt run it
```{r}
getwd() #identify current working direcory
```


What if you find that you are not in the correct working directory? You can change your working directory in two ways. The first is to look at the Session menu at the top of RStudio. Click on Session (next to file on laptop) -> Set Working Directory -> Choose Directory. Then navigate to the folder you want to use as your working directory. 
...
```{r}
x <- 42 #this means x gets the value 42
```
...
```{r}
z=10 #this will work but will create problems eventually
```
...
Broom in the right corner erases everything in the R window
...
```{r}
is.integer(my_integer) #yes bc u changed it right above, imp in plots
```
...
NA is missing number, even if u have a 10000 nums, if u have NA then it will not calc 

4. Recalculate the mean using the following code chunk. Why is the useful?  
```{r}
mean(new_vector, na.rm = TRUE) #removes NA values, instea of TRUE u can also just put T
```