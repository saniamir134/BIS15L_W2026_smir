...
msleep %>% 
  filter(bodywt<10) %>% 
  summarize(mean_bodywt_sm=mean(bodywt)) #summarize used to get quick stats summary
```
...
msleep %>% 
  filter(bodywt>150) %>% 
  summarize(mean_bodywt_lg=mean(bodywt), min_bodywt_lg=min(bodywt), 
max_bodywt_lg=max(bodywt), sd_bodywt_lg=sd(bodywt), total=n())
```
...
msleep %>% 
  ...
  summarise_all(mean) #but gives NAs if. you leave it here
```
...
msleep %>% 
  summarize(across(where(is.numeric),
                   ~mean(.x, na.rm=T))) #.x means all observations, all are not NA
```
...
msleep %>% 
  ...
  summarize(mean_bodywt_primate=mean(bodywt), 
            min_bodywt_primate=min(bodywt),
            max_bodywt_primate=max(bodywt), 
            total=n())
```
...
msleep %>% 
  filter(bodywt>100) %>% 
  n_distinct() #this give you 11 bc in table there are repeats of genus
```
...
msleep %>% 
  filter(bodywt>100) %>% 
  summarize(total=n()) #also shows repeats
```
...
msleep %>% 
  filter(bodywt>100) %>% 
  distinct() %>% #if you add genus inside distinct then it will not give repeats of genus
  pull(genus)
```
...
```{r}
msleep %>% 
  count(vore, sort=T) #sort in descending order
```
vore = x, n = y axis; you can make a plot
```{r}
msleep %>% 
  count(vore) %>% 
