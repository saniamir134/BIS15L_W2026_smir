...
```{r}
heartrate2 %>% 
  separate(patient, into = c("patient", "gender"), sep=-1) #pulled off the last letter but now u have tose underscores
```
...
```{r}
heartrate3 %>% 
  unite(patient_gender, "patient", "gender", sep="_") #u put it back to _ and patient gender
```
...
```{r}
gene_exp <- read_csv("data/beachbugs_long.csv")
```
