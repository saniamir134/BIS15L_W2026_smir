...
```{r}
#cant do this bc there are three variables
```
...
heartrate_long <- heartrate %>% 
  pivot_longer(-patient, names_to="drug", values_to = "heartrate") #dont want patient to move, what are the column names that you want to put the abcd into is drug, values are heartrate
```
...
relig_long
#notice how the income col has a character (k and dollar sign and <), and should have NA in the last cell
```
...
qpcr_tidy <- qpcr_untidy %>% 
  pivot_longer(
    exp1_rep1:exp3_rep3, #u want to pivot these, stay the same
    names_to = c("experiment", "replicate"),
    names_sep = "_", #they are separeted by the _ so R knows where to put the two words
    values_to = "mRNA exp"
  )
qpcr_tidy
```
...
```{r}
qpcr_tidy %>% 
  separate(experiment, into=c("experiment", "e_number"), sep=3) %>% #keeps everything after the 3rd position 
  separate(replicate, into=c("rep", "r_number"), sep=3) %>% 
  select(gene,experiment=e_number, replicate=r_number, `mRNA exp`)
