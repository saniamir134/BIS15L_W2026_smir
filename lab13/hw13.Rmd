...
#NA’s appear to already be represented as -999
```
...
```{r}
total_na <- sum(is.na(amniota_clean))
total_na
```
```{r}
miss_var_summary(amniota_clean)
...
amniota_clean %>% 
  group_by(class) %>% 
  summarize(egg_mass_na = sum(is.na(egg_mass_g))) %>% 
