...
```{r}
polar_bears %>% 
  filter((capture_date>=1980-04-28) & popn=="SB"|popn=="CS") %>% 
  ggplot(aes(x=capture_date, y=popn))+
  geom_line()+
  geom_point(shape=5)+
  scale_x_continuous(breaks=1985-04-28:2016-04-22, minor_breaks = NULL)+
  theme(axis.text.x = element_text(angle=60, hjust=1))+
  labs(title="bears each yr for both populations",
       x="Year",
       y="bears captured")
```
```{r}
...
server <- function(input, output, session) {
  ...
  output$plot <- renderPlot({
    ...
    elephants %>%
      ...
      labs(title = "Range of Elephant Measurements by Sex",
           x = "Sex",
           y = input$y,
           fill = "Sex") +
      theme_light()
