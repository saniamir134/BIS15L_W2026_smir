...
polar_bears %>%
  ...
  labs(title="num of polar bears sampled by sex and pop",x = "pop",y = "num of bears",fill = "sex")
```
...
bears_by_year<-polar_bears_year %>%
  count(year,popn)

ggplot(bears_by_year,aes(x=year,y=n,color=popn)) +
  geom_line() +
  geom_point() +
  labs(title = "num of polar bears sampled each yr by pop",x = "yr",y = "num of Bears",color = "pop")
```
...
```{r}
fasting_year <- un_creat_ratio %>%
  mutate(year=year(capture_date)) %>%
  mutate(fasting = case_when(un_creat_ratio <= 10 ~ "not fasting",un_creat_ratio > 10 ~ "fasting"))
```


```{r}
fasting_counts <- fasting_year %>%
...
```{r}
ggplot(fasting_counts,aes(x=year,y=n,color=popn)) +
  geom_line() +
  geom_point() +
  labs(title="num of fasting polar bears by yr and popn",x="yr",y="num of fasting bears",color="popn")
```
...
```
```{r}
#map2 <- get_stadiamap(bbox2, maptype = "stamen_terrain", zoom = 3)
```
```{r}
#ggmap(map2)
```
...
```{r}
#ggmap(map2) +
#  geom_point(data = polar_bears,aes(x = long, y = lat, color = popn),size = 1.5,alpha = 0.7) +
#  labs(x = "long",y = "lat",title = "loc of polar bears in the study",color = "pop")
```
...
ui<-dashboardPage(
  ...
  dashboardHeader(title = "range of un_creat_ratio for polar bear pop"),
...
server <- function(input, output, session) {
  ...
  output$plot <- renderPlot({
    ...
    polar_bears %>%
      ...
      labs(title = "range of un_creat_ratio by pop",
           x = "pop",
           y = "un_creat_ratio",
           fill = input$fill_var) +
      theme_light()
