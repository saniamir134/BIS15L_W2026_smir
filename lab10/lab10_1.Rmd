...
`geom_col` get to see the count of each and then the plot, when u have x and y
```{r}
life_history %>% 
  count(order,sort=T)
```
...
```{r}
options(scipen=999) #cancels scientific notation for the session, then run the plot again to take it out
```
...
## Scatterplots
Scatter plots allow for comparisons of two continuous variables. Make a scatterplot that compares gestation time and wean mass. relationship btwn things
```{r}
life_history %>% 
...
## Boxplots
Box plots are used to visualize a range of values. So, on the x-axis we have a categorical variable and the y-axis is the range. Make a box plot that compares mass across orders. for range, when u use geom col u can prolly use box plot
```{r}
life_history %>% 
