...
```{r}
injury_count_by_county <- sharks %>%
  count(county, injury) %>%  # counts each injury type
  pivot_wider(
    names_from = injury,
    values_from = n,
    values_fill = list(n = 0)  # replace missing counts with 0
  ) %>%
  mutate(total_incidents = Fatal + Non-Fatal + Unknown)  # sum for total
