...
```{r}
p <- homerange %>% 
  ggplot(aes(x=log10_mass, y=log10_hra)) #storing just the empty plot as p
```

Play with point size by adjusting the `size` argument.
```{r}
p+geom_point(size=4) #jitter helps 
```
```{r}
p+geom_jitter(size=3) #jitter helps 
...
```{r}
p+geom_point(size=3, color="black", alpha=0.8, shape=24, fill="blue")
```
...
homerange %>% 
  ...
  geom_bar()+
  coord_flip()+
  labs(title="obersvations by taxon", x="taxon group", 
       y=NULL) #no label for y
```
...
homerange %>% 
  ggplot(aes(x=taxon, fill=trophic_guild))+
  geom_bar(position="dodge")+ #side by side instead of above where it is ontop 
  coord_flip()+
  labs(title="obersvations by taxon", x="taxon group", 
       y=NULL) #no label for y
```
...
```{r}
homerange %>% 
  ggplot(aes(x=class, y= log10_mass, group=taxon))+ #fill is better bc taxon not showed well
  geom_boxplot()
```
...
```{r}
homerange %>% 
  ggplot(aes(x=class, y= log10_mass, group=taxon, fill=taxon))+ #fill is better bc taxon not showed well
  geom_boxplot()
```
