...
we only use 'tibble()' wne working in the tidyverse. the equivalent command in base R is 'data.frame'. but we wil not use base R
...
```{r}
hot_springs <- read_csv("hsprings.csv") #since getwd is alr in lab3, u odont need to specify where it is in the getwd. if u press the blue arrow in data then you will get the table in data, chr = character; dbl = numeric. so below 2 are characters and 2 are numeric
```
...
```{r}
names(hot_springs) #pressing on the table in data you can see a better looking table
```
...
class(hot_springs$scientist) #changing scientist to a factor instead of a character
```

## Practice
1. Change the class of the springs column to factor.
```{r}
class(hot_springs$spring)
```
...
```{r}
fish <- read_csv("data/Gaeta_etal_CLC_data.csv") #bc we are currently in lab 3 folder, we have to go to data folder and look for this
```
