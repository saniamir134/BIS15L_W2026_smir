...
## Working directories
```{r}
getwd() #by default you're in the directory that u first opened 
#setwd() changes path to documents, desktop, etc
```
...
```{r}
library("tidyverse")
# install.packages("package_name"), this is how you install a new package
```
...
my_vector_sequence #this shows it rigt below instead of in the values section
```
...
```{r}
days_of_the_week[4] #position 4 is thursday in days of week
```
...
```{r}
dim(hbirds) #it should be 3x3, which is correct below   
```

```{r}
str(hbirds) #putting it in strings
```
...
```{r}
write.csv(hbirds,"hbirds_data.csv",row.names=F)
```

data frame is a spreadsheet, organized by vectors. every variable is its own vector
