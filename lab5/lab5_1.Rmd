...
```{r}
select(penguins,species,body_mass_g) # you will see only those two varibales in penguin
```
...
```{r}
select(penguins,contains("mm")) #all mm varibales are pulled out
```
...
```{r}
relocate(penguins,year) #moved year to the front
```
...
```{r}
m_sleep_focus <- select(msleep,where(is.numeric)) #to make new dataframe from what we are given
