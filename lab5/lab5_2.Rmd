...
```
```{r}
filter(penguins,species=="Adelie") #two equal signs, one will not work
```
...
```{r}
filter(penguins,body_mass_g %in% c(5250,5500)) #look into body mass variable and pull out those numbers
```
...
```{r}
filter(penguins,body_mass_g %in% c(5250,5300,5400,5350, 5450,5500))
```

You can also extract observations "near" a certain value but you need to specify a tolerance.  
```{r}
filter(penguins, near(body_mass_g, 5400, tol=100)) #its unti 5450 bc it is NEAR 5400
```
...
```{r}
filter(penguins,species=="Chinstrap" | body_mass_g>4500) #| means or
...
penguins %>% 
  ...
  filter(species=="Gentoo") #pipe command is shift command m, select goes before filter
```
...
penguins %>% 
  ...
  arrange(body_mass_g) #sorted in ascending oder
```
...
penguins %>% 
  select(species,body_mass_g) %>% 
  filter(species=="Gentoo") %>% 
  arrange(desc(body_mass_g)) #in descending order instead of ascending
```

```{r}
penguins %>% 
  select(species,body_mass_g) %>% 
  filter(species=="Gentoo") %>%  #slice pulls out observation in only top and bottom
  slice_max(body_mass_g, n=1) #top observation only, slice_min is only bottom, n=5 would give top or bottom five
```
...
fish %>% 
  ...
  filter(lakeid=="AL" | lakeid=="AR") %>% #you cannot do & because fish isnt from AL and AR at the same time
