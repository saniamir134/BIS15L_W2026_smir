...
```{r}
homerange %>% 
  select(species,trophic.guild,common.name, order,mean.mass.g,mean.hra.m2) %>% 
...
ggplot(data=homerange,
  ...
  labs(title = "relationship between body mass and homerange size",x = "mean body mass(g)", y = "mean homerange size (m^2)")
```
...
homerange %>% 
  ...
  labs(title = "# of Birds and Mammals in the Homerange")
...
Which bird species in the homerange dataset has the largest average homerange size?
```{r}
homerange %>% 
  select(species, class, mean.hra.m2) %>% 
  filter(class=="aves") %>% 
  arrange(desc(mean.hra.m2))
```
