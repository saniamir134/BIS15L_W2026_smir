...
```{r}
library("tidyverse")
library("janitor") #thsi will help with cleaning
...
```{r}
penguins %>% 
  filter(island=="herbivore" | island=="Biscoe") 
```
...
3. Make a plot that shows the relationship between body mass and flipper length. How does this compare among different species? The %>% way
```{r}
penguins %>% 
...
```{r}
penguins %>% 
  distinct(species,island,.keep_all=T) #keep all is keeping all the other columns that have species and island match
```
...
```{r}
penguins %>% 
  mutate(body_mass_kg=body_mass_g/1000) #remember its going to be at the end
```
```{r}
penguins %>% 
  mutate(body_mass_kg=body_mass_g/1000) %>% 
  select(species,body_mass_kg,body_mass_g) #compare the two if it actually changed
```
...
```{r}
penguins %>% 
  mutate(across(ends_with("mm"),~./10)) #~. means you are looking at every observation that ends with mm and you divide by 10 bc its to c, but the name is not changed
```
```{r}
penguins %>% 
  mutate(across(ends_with("mm"),~./10)) %>% 
  select(species,bill_length_cm=bill_length_mm,bill_depth_cm=bill_depth_mm,flipper_length_cm=flipper_length_mm)
```
...
```{r}
glimpse(mammals) #theres a space in wean mass, spaces in the columns,slash
```
...
```{r}
mammals <- mammals %>% 
  clean_names() #changes the column names but the observations stay the same
```
...
```
```{r}
mammals<-read_csv("data/mammal_lifehistories_v2.csv") %>% clean_names() #so everytime you input data you can do clean names so it automatically does it
```
...
```{r}
mammals %>% 
  mutate(across(c(order,family), tolower)) #works on specific columns
```

This will change all columns to lower case. But, notice what happens to numeric columns.
```{r}
mammals %>% 
  mutate(across(everything(),tolower)) #be careful with numeric variables bc it changes their class
```

For this reason, it might be better to use `where` so we can specify only character columns.
```{r}
mammals %>% 
  mutate(across(where(is.character),tolower)) #now numeric columns still say dbr instead of chr
```
...
```{r}
summary(mammals) #why is NA = -999 and how does life span= that, so use if else
```

```{r}
mammals %>% 
  select(genus,species,newborn) %>% 
  arrange(newborn) #also an issue bc -999
```
```{r}
mammals %>% 
  select(genus,species,newborn) %>% 
  mutate(newborn_new=ifelse(newborn== -999.00,NA, newborn)) #give newborn a new name newborn new and if you see -999 in newborn put NA but if u dont, leave it as newborn
```
...
```{r}
mammals <- mammals %>% 
  mutate(across(c(mass, wean_mass,gestation,max_life,newborn,weaning,litter_size,afr,litters_year), ~ifelse(.==-999,NA, .)))
```
...
```{r}
mammals %>% 
  ggplot(mapping=aes(x=gestation,y=log10(newborn)))+ #log it bc the graph looks weird
  geom_point()+
